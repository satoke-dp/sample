...
sequenceDiagram
    ...
    Hire->>IssuerFE: 9. 『MySovで本人確認を行う』ボタン、QRコードが表示されるので、ボタンをクリックまたはQRコードを読み込む。
    IssuerFE->>MySov: 10. MySovに遷移 (Challengeを含む要求)

    %% 6. 署名と提示 (VP Presentation)
    Hire->>MySov: 11. マイナンバーVCを選択し、Nonceを含めてVPに署名 (Holder秘密鍵)
    Note right of MySov: VPをvp_tokenに格納
